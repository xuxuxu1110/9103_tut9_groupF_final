...
function initializeArtwork() {
  ...
  while (wheels.length < numWheels && currentAttempts < maxAttempts) {
    ...
    for (let other of wheels) {
      ...
      // Allow some overlap (e.g., up to 50% of the smaller radius)
      // Original artwork has significant overlaps, so we need to allow for it.
      const overlapThreshold = min(candidateRadius, other.radius) * 0.4; // Allow 40% overlap
  ...
  // Generate connectors between nearby wheels (inspired by yzh's connection logic)
  for (let i = 0; i < wheels.length; i++) {
    for (let j = i + 1; j < wheels.length; j++) {
      ...
      if (d < (w1.radius + w2.radius) * 1.3) { // Connect if distance is within 1.3 times their combined radii
        connectors.push(new Connector(w1, w2, random(colorPalettes)[0])); // Pass a random color from a palette
      }
    }
  }
}
...
class Connector {
  ...
  display() {
    ...
    const linkSize = 10;
    const numLinks = floor(distBetweenWheels / (linkSize * 1.5)); // Calculate number of links
    if (numLinks > 0) {
